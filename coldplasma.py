...
S = cold_plasma_oscillations(f"CO_LINEAR", qmratio=qmratio, plasma_frequency=plasma_frequency, NG=NG,
...
S = cold_plasma_oscillations(f"CO_NONLINEAR", qmratio=qmratio, plasma_frequency=plasma_frequency, NG=NG,
                             N_electrons=N_electrons, epsilon_zero=epsilon_zero, push_mode=push_mode,
                             push_amplitude=0.8,
                             save_data=False, T = T, scaling=scaling, c=c).run()
plots(S, *args, animation_type = animation.OneDimAnimation, alpha=0.3)
